...
class Venta extends Conectar
{
    ...
    public function insert_venta_detalle($vent_id, $prod_id, $prod_pventa, $detv_cant, $detv_descuento)
    {
        $conectar = parent::Conexion();
        $sql = "CALL SP_I_VENTA_02_DESCUENTO (?,?,?,?,?)";
        $query = $conectar->prepare($sql);
    ...
    // public function get_stock_total_by_id($prod_id)
    // {
    //   $conectar = parent::Conexion();
    //   $sql = "CALL SP_L_STOCK_TOTAL_BY_ID (?)";
    //   $query = $conectar->prepare($sql);
